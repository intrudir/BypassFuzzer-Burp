...
- `bypass-options` configures headers, Collaborator availability, cookie fuzzing, concurrency, throttle response codes, and execution posture through `Options...`.
...
- `bypass-results` sorts results, opens Request/Response and original/baseline viewers, highlights rows, copies TSV, and retries queued throttled requests.
